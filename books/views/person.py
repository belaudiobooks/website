...
def person_detail(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    context = {
        'person': person,
        'authored_books': [BookForPreview.with_all_narrations(book) for book in authored_books],
        'translated_books':
        [BookForPreview.with_narrations_from_translator(book, person) for book in translated_books],
        'narrated_books':
        [BookForPreview.with_narrations_from_narrator(book, person) for book in narrated_books]
    }
