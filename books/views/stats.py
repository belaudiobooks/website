...
def birthdays(request: HttpRequest) -> HttpResponse:
    ...
    for person in people[:30]:
        next_birthday = datetime.date(
            now.year, person.date_of_birth.month, person.date_of_birth.day
        )
        if next_birthday < now.date():
            next_birthday = datetime.date(
                now.year + 1, person.date_of_birth.month, person.date_of_birth.day
            )
        people_with_info.append(
            {
                "date_of_birth": person.date_of_birth,
                "person": person,
                "age": now.year - person.date_of_birth.year,
                "stats": "%d - %d - %d"
                % (
...
def digest(request: HttpRequest) -> HttpResponse:
    ...
    context = {"last_3_months": [item[1] for item in last_3_months]}
