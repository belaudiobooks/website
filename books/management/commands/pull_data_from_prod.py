...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dir in ["covers", "photos", "icons", "logos"]:
            ...
            subprocess.run(
                ["gsutil", "-m", "rsync", "-d", f"gs://books_media/{dir}", full_dir],
                check=True,
            )
        ...
        all_books = (
            Book.objects.using(REMOTE_DB)
            .all()
            .prefetch_related("authors", "translators", "tag")
            .order_by("uuid")
        )
        all_narrations = (
            Narration.objects.using(REMOTE_DB)
            .all()
            .prefetch_related("narrators", "publishers")
            .order_by("uuid")
        )
        all_link_types = LinkType.objects.using(REMOTE_DB).all()
        all_links = Link.objects.using(REMOTE_DB).all().order_by("uuid")
        all_publishers = Publisher.objects.using(REMOTE_DB).all().order_by("uuid")
        all_objects = list(
            chain(
                all_people,
                all_tags,
        ...
        with open("data/data.json", "w", encoding="utf8") as f:
            django.core.serializers.serialize("json", all_objects, indent=2, stream=f)
