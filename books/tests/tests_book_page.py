...
class BookPageTests(WebdriverTestCase):
    ...
    def _get_section(
        self, book_or_narration: Union[models.Book, models.Narration]
    ) -> WebElement:
        if isinstance(book_or_narration, models.Book):
            return self.driver.find_element(
                By.CSS_SELECTOR, '[data-test="book-section"]'
            )
        else:
    ...
    def _check_persons_present_and_clickable(
        self,
    ...
    def _check_narration_links_present(
        self,
        expected_section: Union[models.Book, models.Narration],
    ...
    def _create_narration(
        ...
        narration = models.Narration(
            language=language, book=self.book, paid=False, date=date
        )
        narration.save()
        narration.links.add(
    ...
    def test_has_livelib_clickable_link(self):
        ...
        self.assertEqual(
            "https://www.livelib.ru/book/pershaya-kniga", elem.get_attribute("href")
        )
    ...
    def test_multiple_narrations_ordered_correctly(self):
        ...
        self.book.narrations.set(
            [
                self._create_narration(
                    models.Language.RUSSIAN,
                    self.fake_data.person_ales,
                    date=date(2020, 1, 3),
                ),
                self._create_narration(
                    models.Language.BELARUSIAN,
        ...
        self.assertIn(self.fake_data.person_viktar.name, narrators_sections[0].text)
        self.assertIn(self.fake_data.person_bela.name, narrators_sections[1].text)
        self.assertIn(self.fake_data.person_ales.name, narrators_sections[2].text)

    def test_book_description_merged_with_narration_when_single_narration(self):
        ...
        description = self.driver.find_element(
            By.CSS_SELECTOR, '[data-test="book-description"]'
        ).text
