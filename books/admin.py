...
class IncompleteBookListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [
            (key, f"{value} ({self._get_books_for_reason(queryset, key).count()})")
...
class IncompletePersonListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [
            (key, f"{value} ({self._get_people_for_reason(queryset, key).count()})")
            for key, value in reasons.items()
        ]
...
class IncompleteLinksSetFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [
            (key, f"{value} ({self._get_narrations_for_reason(queryset, key).count()})")
            for key, value in reasons.items()
        ]
    ...
    def _get_narrations_for_reason(self, queryset, reason):
        ...
        if reason == "no_google_play":
            return queryset.filter(links__url_type__name="rakuten_kobo").exclude(
