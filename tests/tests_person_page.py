...
class PersonPageTests(WebdriverTestCase):
    ...
    def _check_books_present(self, section_name: str, books: list[models.Book]):
        section = self.driver.find_element(
            By.CSS_SELECTOR, f'[data-test="{section_name}"]'
        )
        ...
        for book in books:
            ...
            self.assertEqual(
                1,
                len(books_links),
                f'Book "{book.title}" not found or found more than once',
            )
            self.assertEqual(
                f"/books/{book.slug}", books_links[0].get_dom_attribute("href")
            )
    ...
    def test_shows_correct_books(self):
        ...
        self._check_books_present(
            "books-translated", [book_translated_1, book_translated_2]
        )
