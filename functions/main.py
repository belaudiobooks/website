...
def generate_resized_image_name(original_file: str, size: int) -> str:
    assert (
        original_file.count(".") == 1
    ), f"Invalid name: {original_file}. Has more than one dot."
...
def resize_image(
    ...
    for size in SIZES_TO_GENERATE:
        ...
        # the original image.
        if (
            resized_name in blobs
            and blobs[resized_name].metadata is not None
            and blobs[resized_name].metadata[ORIGINAL_IMAGE_CRC32C]
            == original_image.crc32c
        ):
        ...
        if DRY_RUN:
            ...
            resized_blob.metadata = {ORIGINAL_IMAGE_CRC32C: original_image.crc32c}
...
def delete_unused_resized_images(blobs: dict[str, storage.Blob], used_images: set[str]):
    ...
    for ind, blob in enumerate(blobs.values()):
        ...
        if (
            blob.metadata is None
            or blob.metadata[ORIGINAL_IMAGE_CRC32C] is None
            or name in used_images
        ):
            continue
        if DRY_RUN:
...
def resize_images(request):
    ...
    used_images = set()
    original_images = [name for name in blobs.keys() if is_original_image(name)]
