...
def _get_image_name(
    folder: str,
    instance: Union["Person", "Book", "Publisher", "Narration"],
    filename: str,
) -> str:
    """Builds stored image file name based on the slug of the model."""
    slug = isinstance(instance, Narration) and instance.book.slug or instance.slug
...
class Person(models.Model):
    ...
    gender = models.CharField(
        _("Person gender"), max_length=20, choices=Gender.choices, blank=False
    )
    date_of_birth = models.DateField(
        _("Date of birth"), auto_now_add=False, blank=True, null=True
    )
...
class Tag(models.Model):
    ...
    slug = models.SlugField(
        _("Tag slug"), max_length=100, allow_unicode=True, blank=True
    )
...
class BookManager(models.Manager):
    def active_books_ordered_by_date(
        self, prefetch_fields=["authors"]
    ) -> models.QuerySet:
        return (
            self.prefetch_related(*prefetch_fields)
...
class Book(models.Model):
    ...
    uuid = models.UUIDField(
        _("Book ID"), primary_key=True, default=uuid.uuid4, editable=False, unique=True
    )
    title = models.CharField(_("Book Title"), max_length=100, default="")
    title_ru = models.CharField(_("Book Title in russian"), max_length=100, default="")
    description = models.TextField(_("Book Description"), blank=True)
    description_source = models.CharField(
        _("Book Description Source"),
        blank=True,
        max_length=500,
        default="",
        validators=[
    ...
    status = models.CharField(
        _("Status"), max_length=20, choices=BookStatus.choices, blank=False
    )
    livelib_url = models.CharField(
        _("LiveLib URL"), max_length=256, blank=True, default=""
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if Book.objects.filter(slug=self.slug).exclude(uuid=self.uuid).count() > 0:
            for i in range(2, 100):
                ...
                if Book.objects.filter(slug=new_slug).count() == 0:
                    self.slug = new_slug
                    break
        super().save(*args, **kwargs)
...
class Publisher(models.Model):
    ...
    logo = models.ImageField(
        upload_to=functools.partial(_get_image_name, "logos"), blank=True, null=True
    )
...
class Narration(models.Model):
    ...
    book = models.ForeignKey(Book, related_name="narrations", on_delete=CASCADE)

    paid = models.BooleanField(_("Is narration paid?"), default=False)

    language = models.CharField(
        _("Language"), max_length=20, choices=Language.choices, blank=False
    )

    duration = models.DurationField(_("Duration"), blank=True, null=True)

    publishers = models.ManyToManyField(
        Publisher, related_name="narrations", blank=True
    )
    ...
    cover_image_source = models.CharField(
        _("Cover Source"),
        blank=True,
        max_length=500,
        default="",
    ...
    preview_url = models.CharField(
        _("Preview URL"), max_length=100, blank=True, default=""
    )
...
class LinkType(models.Model):
    ...
    caption = models.CharField(
        _("Link Caption"), max_length=100, blank=True, default=""
    )
    icon = models.ImageField(upload_to="icons", blank=True, null=True)
    disabled = models.BooleanField(_("Disabled"), default=False)
    url_regex = models.CharField(_("Url regex"), max_length=100, blank=True, default="")
    availability = models.CharField(
        _("Availability"), max_length=50, choices=LinkAvailability.choices, blank=False
    )
...
class Link(models.Model):
    ...
    uuid = models.UUIDField(
        _("Link Id"), primary_key=True, default=uuid.uuid4, editable=False, unique=True
    )
    url = models.URLField(_("URL"), max_length=1024)
    url_type = models.ForeignKey(
        LinkType, related_name="link_type", null=True, on_delete=SET_NULL
    )
    narration = models.ForeignKey(
        Narration, related_name="links", on_delete=CASCADE, null=True
    )
